...
- **改动历史（卡片上的「查看 diff」）**：`edit` 显示的是**该次调用自己的补丁视图** —— 改动附近 4 行上下文，多个改动段之间用「⋯（中间省略）」隔开，**不是整文件**；它由 pi 写进会话文件，所以重启 VS Code 后仍可打开。`write` 显示的是**整文件前后**，前后内容只活在**本次 VS Code 进程**的内存里 —— 重启后同一张卡片显示「本次会话不可用」。内存上限：`edit` 保留最近 100 条、`write` 最近 20 条（单条超过 2 MiB 不保留），超出上限后**最早**的卡片显示「较早的改动记录已清理」。失败的工具调用（例如 `edit` 没匹配上）不会给 diff 入口。
