...
## 7. 人工验收 —— Mac（**压缩后：用户只做 2 个动作**）

**标记规则（本节适用，后续切片也适用）**：每条验收都必须标 `【自动】` 或 `【人工】`；
`【人工】` **只允许**属于以下四类之一 —— 不符合却写成 `【人工】` 的，就是在偷懒，应当驳回：

1. **排版 / 外观**（无头 DOM 没有排版引擎，见 §6 的红线）
2. **真焦点 / 真键盘**
3. **真进程**（真的中止、真的跑 bash）
4. **Windows 路径与 shell 形态**（目标机器独有）

| 原项 | 谁来覆盖 | 归属 |
| --- | --- | --- |
| 版式的**位置关系**（`#status` 在输入框之上、`#meta` 在其下） | 测试台结构断言 | 【自动】 |
| 版式在**真侧边栏**里的样子（挤压 / 错位 / 空白） | 无头 DOM 测不了 | 【人工】动作 ①（一张截图） |
| 点模型段 / 等级段发出对应请求 | 测试台断言 | 【自动】 |
| QuickPick 弹出、选中后**真的切了模型** | 宿主侧假 `vscode` 断言 + `controller-check` 真切换 | 【自动】 |
| 选完能**直接打字**（焦点回到输入框） | 真焦点 | 【人工】动作 ①（截图后顺手打一行） |
| 等级列表**带空洞**、随模型变（flash 有 `low`、v4-pro 没有） | 渲染断言（喂假模型对象）+ §3.5 的探针已实测真值 | 【自动】 |
| 一轮对话后百分比**变化** | `controller-check` 真跑一轮后读 `getContextUsage()` | 【自动】 |
| `>70%` 变黄 / `>90%` 变红 | 阈值函数单测；**造 70% 上下文不现实** | **未覆盖**（记录在案，不让用户造数据） |
| 状态栏项创建 / 更新 / 隐藏 | 宿主侧假 `vscode` 断言 | 【自动】 |
| **滚动回归**（贴底跟随 / 向上滚不被拽回） | 无头 DOM 无排版引擎，`scrollHeight` 恒 0 —— S3 栽过 | 【人工】动作 ② |
| 队列行为（取回编辑 / 追加 / 转向） | 测试台断言 | 【自动】 |
| 重开面板后状态恢复 | S2 已验收同类；快照/重放有自动断言 | 【自动】 |
...
- 顶部**没有**状态行（转录区因此更靠上）
- **输入框上方**：状态行（空闲时**不占位置**）
- **输入框下方**：形如 `deepseek-v4-flash • high • 0.0%/1M`
- 转录区**没有被挤扁**、按钮没换行、输入框高度正常
- 点模型段 → QuickPick 弹出 → 选一个 → **紧接着打字能打进去**（焦点）

**② 发一条长回复，流式中滚一下，再点「中止」**：

- 向上滚时不被拽回底部；回到底部后继续跟随
- 中止后：卡片变 `✗`、已流出的行还在、**输入框上方的**状态行回到空闲

## 8. 人工验收 —— Windows（并入下一次 Windows 轮次，约 3 分钟）

| # | 步骤 | 期望 | 标记 |
| --- | --- | --- | --- |
| W0 | `Pi: Run Self-Test` | `GATE PASS`（本步动了协议与 webview，必须先自证没破坏受限机路径） | 【人工】真进程 |
| W1 | 版式 + 元信息行 + 状态栏项（截图） | 与 Mac 一致；状态栏项显示模型 + 百分比，点击能开选择器 | 【人工】目标机外观 |
| W2 | 切模型（`deepseek-v4-flash` → `...-vision-exp`）后继续对话 | 切换生效；选完能直接打字 | 【人工】真进程 + 真焦点 |
| W3 | 等级段 | flash 列表含 `low`；切到 `v4-pro` 后 `low` 消失 | 【人工】目标机 |
| W4 | 重开 webview 后元信息行 | 仍显示正确的模型（不是"未选择模型"） | 【人工】目标机 |
