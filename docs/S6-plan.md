...
### 12.1 自动检查（**全部绿**，2026-09-14）

| 检查 | 结果 |
| --- | --- |
| `npm run typecheck`（两套 tsconfig） | ✅ |
| `npm run self-test` | **9/9**（protocol 112 / render 114 / tool-text 91 / settings-check 16/16 / webview-dom 75 / host-check **95/95**） |
| `npm run check:controller`（真模型，不进 CI） | **98/98**（A4 真对话、A5 真 auth.json、A13/A14、A12 正反两条 + 真 spawn `pi -c` 互通） |
| `npm run check:gate`（无头跑 `Pi: Run Self-Test`，不进 CI） | **15/15 GATE PASS**（T5c/T12/T13 都 PASS） |
| `npm run package` + `check-vsix` | **0.1.8：338 文件 / 5.75 MB**，文件清单与 0.1.7 逐个相同 |

### 12.2 提交切分（实际）

计划/评审期：`1dbe07a`（初稿）→ `17eeaf7`/`d65f9ff`/`4afa62d`（第 1–3 轮评审）→ `ce4fc5a`（第 3 轮配额中断记档）→ `7d5ee81`（第 4 轮收敛）。

实施期（一步一提交）：

| 提交 | 内容 |
| --- | --- |
| `14b0e6e` | 第 1 步：声明三项设置 + 读取层 + 变更提示（A1/A2/A3） |
| `8c3f3a5` | 第 2 步：agentDir 贯通（A9 实现、A13/A14） |
| `f388014` | 第 3–4 步：清 key 核心 + Set API Key 补完（A5/A6 纯函数） |
| `4fea5c7` | 第 5 步：A4 真对话 + 环境清理提取 |
| `a6a9182` | 第 6 步：Refresh Catalog + A8/A12 |
| `6bbe918` | 第 7 步：T13 + 纯函数（A11） |
| `8c54db9` | 第 8 步：A9/A6b/A7 欠账 + 双语文档 |
| `184adcd` | 第 9 步：0.1.8 + 可复现的无头闸门 |
| `c97a7c6` | M1 说明补充（空目录需要先设 key） |
| `6ca843d` | M1 抓到：通知里的 markdown 记号 |
| `b89abbf` | **M1 抓到：读设置时 section 与 key 搞反**（真 bug） |

`638532b` 是计划调整（项目信任流程排到 S8，用户决定），不属实施步骤。

### 12.3 人工验收

**Mac（用户 2 个动作）—— ✅ PASS（2026-09-14）**

| 项 | 结果 |
| --- | --- |
| M1 真进程：改 `jerrypi.agentDir` → 重载 → 发消息 → 删设置 → 重载 | ✅。判据行先为 `[jerrypi] agentDir=/tmp/jerrypi-m1-agent（来源：设置）`，会话落在 `${temp}/sessions/--Users-fengrui--/`；收尾后回到 `来源：默认` 且旧会话（`~/.pi/agent` 那份）回来了 |
...
| 项 | 为什么 | 记在哪 |
| --- | --- | --- |
| 项目级设置信任流程 | **不在 S6 范围**（早期计划指针写错）；用户 2026-09-14 拍板排到 S8，具体设计待 S8 计划期 | `PLAN.md` §6 的 S8 注 · `STATUS §2` |
| `jerrypi.proxy` 第二层（自带 undici） | Q2 决定不做；T13 只报告身份不判定 | §3.4 · README 已知限制 |
| T13 内容的“正确性” | 只报告不判定（T4 已证明网络可用） | §3.4 · §6 A11 |
| R10（自定义 agentDir + 盘符大小写） | 按 §3.7 **接受**，只记限制 | §3.7 · `pi-traps` 第 18 条 |
| 滚动行为 / `>70%` 颜色 / 多写者 / 符号链接 | 沿用 S4/S5 的未覆盖清单（本阶段未动那些路径） | S5-plan §12.4 |

## 13. 待用户拍板

**已拍板（2026-09-13）**：Q1–Q9 全按默认值执行（见 §4）。
**另行拍板（2026-09-14）**：项目级设置的信任流程不在 S6 范围内（早期计划指针写错），**排到 S8**（见 `PLAN.md` §6 的 S8 注）。
