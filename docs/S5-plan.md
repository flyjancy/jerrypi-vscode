...
6. **发布 0.1.7 预发布**（STATUS §4 的流程：`npm run package` → `check-vsix` → 用户手动上传 →
   `compare-vsix.mjs 0.1.7` → `git tag -a v0.1.7`）。
7. **Mac 验收（§7）→ Windows 验收（§8）→ 回填 §12**。
...
**这两条要不要补一轮评审？** 我的判断：**不补**。U1 是产品取舍（已由用户拍板），U2 是划界；
两者都不新引入事实论断 —— 除了 D7 里那句"Codex/Claude 的插件能把会话跑在后台"，
