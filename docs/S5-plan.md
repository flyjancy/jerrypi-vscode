...
> 状态：**已关闭（2026-09-13，0.1.7）** —— 三轮评审 → 全部拍板 → 按 §9 实施完 1–8 步 →
> Mac/Windows 双端验收通过 → 发布 0.1.7（Marketplace 逐文件核验一致）。结果见 §12。
> **进度只在 `docs/STATUS.md`**（本文档不记进度）。
>
> 计划期的状态（留档）：**三轮评审已完成**（第 1 轮 5B/5S/6N、第 2 轮 4B/5S/6N、第 3 轮只核对转写；裁决与差异均见 §10）。
...
| **macOS 上的真实互通**（CLI 真能打开我们写的会话） | — | **写进 `controller-check`（不新增脚本** —— 保住 `npm run self-test` 里"12 个检查脚本"那条）：在临时 cwd 下用我们的代码路径写一条带 assistant 的会话，然后真 spawn `pi -c -p "…"`（`cwd=<那个临时目录>`）→ 走它**自己的默认查找**（连路径都不用拄）、断言退出码 0 且回答里提到我们写进去的标记。要网络与真模型 → 不进 CI，但**跑由我跑**（§10.1 的 U5）。找不到 `pi` / 超时 → SKIP 并注明（受限机与 CI 就是这类） | — |
...
> **纪律（S4 留下来的；我初稿违反了，已改回）**：`【人工】` 只允许属于四类 ——
> ①排版/外观 ②真焦点/真键盘 ③真进程 ④Windows 路径与 shell 形态；
> **不属于这四类的一律自动化**，而且每条留下的人工项都要写清"为什么自动化不了"。
> S5 初稿排了 **3 个 Mac + 4 个 Windows**，比 S4（2 个 Mac）还多 —— 其中两条本来就不该给用户，
> 已搬走（见 §10.1 的 U5）：**CLI 互通改由我自动跑**，**重启恢复挪去 Windows**。

| 项 | 谁来覆盖 | 归属 |
| --- | --- | --- |
| 新会话文件落在编码目录 | `controller-check` + 自测 T10 | 【自动】 |
| **CLI 真能打开扩展写的会话**（G3 核心） | **`controller-check` 新增一段：真 spawn `pi -c -p …`**（要网络与真模型 → 不进 CI，但**跑由我跑**） | 【自动】（U5 改） |
| 列表内容/顺序/当前项、切换后转写与白名单重建 | `controller-check` + `host-check` | 【自动】 |
| 切换后 `Reload Webviews` 仍正确 | 快照断言（S2 已验同类） | 【自动】 |
| README 已知限制与面板实际行为一致 | **我自己在 Mac 上对着面板核**（不占用户动作） | 【自动】（U5 改） |
| 重启后自动恢复上一会话 | → 挪到 **Windows W1**（G3 的判据本来就写"受限机重启后恢复"；Mac 重做一遍是重复劳动） | 【人工】Windows |
| 忙时确认弹窗的**真实交互**（点了到不到底切） | ②真焦点 + ③真进程：原生 QuickPick/模态，无头 DOM 测不了"弹窗长什么样"，更测不了"确认后真的切了" | 【人工】动作 ① |
| 元信息行多一段后的**版式**（窄栏下换行/挤压） | ①排版：无排版引擎 | 【人工】动作 ②（顺手看，不单独跑一趟） |

### 你要做的 2 个动作（**同一个窗口里就能做完，不用重启、不用换机器**）

**① 真机交互**（一次试完"新建 / 忙 / 闲"三条路径）：

0. 面板起来后**先点会话名 → 列表里点「新建会话」** —— 两个理由：
   ① 它本身就该验（面板应当**清空**，这是 S5 修掉的那个 bug）；
   ② **避开一个坑**：这个仓库的会话目录里最近被写过的会话很可能正是"你和 pi 在终端/别的窗口里
   正在聊的那段"，直接往它里面发消息会让两边的写入缠在同一份 `.jsonl` 上（R8）。
1. 在新会话里发一条**长回复**（例如"数 1 到 100，每行一个数字"）；
2. **生成中**点 `Pi: Resume Session` →
   - 点「取消」→ 会话没变（元信息行的会话名段不动）；
   - 再点一次、点「继续」→ 切过去了（旧会话那一轮已被中止并已落盘）；
3. **空闲时**再点 `Pi: Resume Session` → 选列表里**另一个**会话 →
   转写换成那个（**不带旧内容**），而且**接着直接打字能打进去**。

**② 顺手看一眼版式**（就在上面那个窗口里）：把侧边栏拖窄 ——
`会话名 · 模型 · 等级 · 0.0%/1.0M` 应**不换行、不被挤压**。

> **互通怎么验了**（不再占你的动作）：`controller-check` 里新增的那段会在临时 cwd 里
> 用我们的代码路径写一条会话，然后 `cd <那个临时目录> && pi -c -p "…"` ——
> 用的是**你终端那份 pi**，而且走它自己的默认查找（连路径都不用抄）。
> 结论会写在 `controller-check` 的输出里；我会在 §12 里把那行原文贴出来。

---

## 8. 人工验收 —— Windows（**2 项**，初稿是 4 项，见 §10.1 的 U5）

| # | 步骤 | 期望 | 标记 |
| --- | --- | --- | --- |
| W0 | `Pi: Run Self-Test`（点一下命令、把结果贴给我就行） | **`GATE PASS`，12 个 gating 项全 PASS**（T5c/T12 是 advisory）。⚠️ **不要写 `11/11` 这种格式**：`computeGate()` 返回的就是裸字符串，那个数字是人工数的。新增的 T10/T11 在**受限机路径**上要过（Windows 路径含 `\` 与 `C:`，正是编码规则的最坏情况）；T12 在这台机器上必然 **SKIP**（没有 `pi`），这是对的。**顺手（评审 N6）**：VS Code 给的盘符是**小写** `c:\…`、shell 报大写 —— 把两种写法贴给我 | 【人工】③真进程 + ④Windows 路径 |
| W1 | 重启 VS Code → 面板；然后点一次 `Pi: Resume Session`，选另一个会话 | ① 面板**自动**回到上一会话（转写完整）；② 列表里有刚聊的那条、选中后转写换成那个（不带旧内容），**能直接打字** | 【人工】③真进程 + ②真焦点 |

**为什么只剩两项、其余去哪了**（不写清就会看起来像漏了）：

| 原项 | 处置 | 理由 |
| --- | --- | --- |
| 旧 W2（切会话 + 打字） | **并入 W1** | 同一台机器同一个窗口，没理由分两个动作 |
| 旧 W3（README 逐条核对） | **搬到 Mac、由我核** | 它不是"Windows 独有现象"，而文档与行为的对照我可以自己对着面板做 |
| CLI 互通 | **自动化（我跑）** | 受限机没 `pi` → 本来就不在这里验；Mac 那边已改成 `controller-check` 自动跑 |
| `MissingSessionCwdError` 的提示文案 | 不验 | `controller-check` 覆盖（构造一个 cwd 已删的会话文件）；真机上要删目录才能造，不值一个动作 |
...
2. **启动即恢复（D4）+ 会话替换后重放（D6，含修掉 §3.7 缺口 A）**
   ...
   **会话建立与每次替换后 Output 各记一行 `[controller] 会话文件：<绝对路径>`**
   （`session.sessionManager.getSessionFile()`，`agent-session.d.ts` 里是 public；host-check 里断言它出现过）。
   ⚠️ U5 之后它**不再是**任何人工动作的前置（互通改成 `controller-check` 自动跑了）——
   留着是因为"我的会话存哪了"是个真问题（S4 的 §12.5 就是这么冒出来的），成本就一行日志。
...
6. **打包 + 自测 + Mac 验收（§7，用户 2 个动作）**：`npm run package` → `node scripts/check-vsix.mjs` →
...
### 第 6 步（打包 + 自测 + Mac 验收，2026-09-13）

| # | 事项 | 说明 |
| --- | --- | --- |
| 6-1 | 版本 → **0.1.7**，`npm run package` + `check-vsix` | 第一次打出来是 **340 文件 / 6.5 MB**（比基线多 0.76 MB）—— 多的是用户 77196c1 提交的两个 **logo 候选**（`cheese-mouse.png` 一张就 800KB，占全包 12%，而目前没有任何地方引用它们） |
| 6-2 | `.vscodeignore` 排除两个候选 → 回到 **338 文件 / 5.75 MB** | 与 `.pi/settings.json` 同一类问题（开发素材进包）；注释里写明"将来选定并真的接进 `package.json` 时把那一行删掉" |
| 6-3 | **`.vscode/launch.json` 给 F5 加一个参数**：`${workspaceFolder}` | 默认的调试宿主**不打开任何文件夹**，于是面板的 cwd 会退回用户主目录（`workspace.ts:18`），验收看到的是另一套会话目录 —— 加这个参数才能验到"本仓库的会话"。这是**为验收服务的**，也让 F5 更接近真实用法 |
| 6-4 | §7 动作① 加了第 0 步："先点新建会话" | 两个理由：① 它本身就该验（面板清空 = S5 修掉的那个 bug）；② 这个仓库最近被写过的会话很可能就是"用户与 pi 正在聊的那段"，直接发消息会让两个写者缠在同一份 `.jsonl` 上（R8） |
| 6-5 | 顺手修掉两处**门禁抖动**（§11 的 1-5 记过这个弱点） | ① 第一条 prompt 加"不要调用任何工具"——"只剩一个 assistant 节点"的前提应当由提示词保证，不能靠运气；② "中止后会话仍可用"改成断言**行为**（又跑完一轮 + 非空 + 不忙），不再断言模型是否真的回了那两个字母 |

### 第 6 步 · Mac 验收期（用户报的第一个问题，2026-09-13）

**用户报的**：确认框长这样（截图），"**原会话会被中断，但是恢复回去还在被中断的样子**"。

**先查证（无头复现，不猜）**：写一个探针跑"切走 → 点继续 → 再切回旧会话"，并且把重放的快照
**喂给真的 webview** 看面板上到底长什么样。结论是**重放没有 bug**：

| 时刻 | 快照 |
| --- | --- |
| 切走前（旧的实时态） | 5 条 item、`busy=true`、队列 1 条 |
| 点「继续」之后（新会话） | **0 条**、`busy=false`、队列空 ✓ |
| 又切回旧会话之后 | 7 条、`busy=false`、队列空 ✓ |

切回后面板上是这三行（面板 DOM 里读出来的原文）：

```
✗ bash  for i in $(seq 1 40) …（还有 3 行）
s                                 ← 排队的那条「转向」被 pi 注入成了旧会话里的一条 user 消息
This operation was aborted        ← pi 给"被中止的那一轮"的原文错误
```

也就是说：**"还在被中断的样子"= 一次被中止的真实痕迹**，三条全是 pi 自己的记录
（工具 ✗ + 排队消息进去但没被回答 + 中止错误）。这不是卡住（`busy=false`、队列已空、
`✗` 卡片是**结束态**而不是 `运行中…`）。

**但确实有一条该改**：确认框只说了"这一轮会被中止"，**没说排队里的消息也会被一起处理掉**
—— 而那正是用户刚刚做过的事（截图里 `待处理 1 条 / 转向: s`）。

| # | 事项 | 说明 |
| --- | --- | --- |
| 6-6 | 确认框文案补全："切换会话会中止这一轮（它会被保存到原会话里，**排队中的消息也会一并被处理**）" | host-check +1 条断言（文案必须同时提到"中止这一轮"与"排队"），57/57 |

**用户 2026-09-13 的决定：① ② 都不做。** 结论是"保持忠实记录 + 不跨会话搬文字"：

| 选项 | 决定 | 理由（记下来，免得再问一遍） |
| --- | --- | --- |
| ① 把中止的红字二次加工成"已被中止" | **不做** | 那是 pi 的原文；改了就是我们替它说话，而"记录忠实"优先 |
| ② 切走时把排队消息退回输入框 | **不做** | 那段文字属于旧会话的上下文；搬到新会话的输入框更容易让人困惑 |

**第 6 步的门禁**：typecheck ✅｜`npm run self-test` 9/9｜`check:controller` **81/81**｜
`host-check` 56/56｜VSIX **338 文件 / 5.75 MB**｜闸门（下一步与 Mac 验收一起跑）。

### 第 5 步（文档，2026-09-13）

| # | 做了什么 | 说明 |
| --- | --- | --- |
| 5-1 | README（中英双语）：会话管理 `计划中（S5）` → **已实现**，并把能力写清（启动自动恢复 / `Pi: Resume Session` / 点会话名 / 忙时先问） | 中英两处都要改（README 是双语的） |
| 5-2 | README 已知限制**删掉一条、新增三条** | 删："窗口重载会开始新会话"（已不成立）；增：① 会话"看不到"的两种情况（≤0.1.6 的平铺 + CLI 的两个搬家开关）② **启动会自动接过上一会话**（含终端那次；判定是"文件最后被写过"）③ 忙时切换会先问一句 + 同一个会话不要两边同时写 + 路径写法要一致 |
| 5-3 | README 顶部状态行：`S0–S2 已实现` → **`S0–S5 已实现`** | 早就过期了（S3/S4 完成时没改） |
| 5-4 | `docs/PLAN.md` 的 S5 行**改正**（`<agentDir>/sessions` → `<agentDir>/sessions/--<编码 cwd>--`）并加一条修正注（写清错在哪、实测数字、修法） | 计划里那处误解就是当年评审 D5-N3 的落点 |
| 5-5 | `docs/PLAN.md` 的 D5-N3 评审记录**加注**："结论错，2026-09-13 由 S5 修正"，并写下教训 | 不改写历史，但让后来读评审记录的人不会被那条**已被 ACCEPT 的错误结论**带回去 |
| 5-6 | `docs/PLAN.md` §5.1 目录清单：修正 `sessions.ts` 的说明，补 `host/modelPicker.ts`、`host/sessionPicker.ts`、`host/sessionActions.ts` | 前两个是 S4/S5 新增但一直没进清单 |

**第 5 步的门禁**：只改文档与一行诊断代码（R9 的日志）→
typecheck ✅｜`npm run self-test` 9/9｜`check:controller` **81/81**（+2：R9 的两个分支）。
