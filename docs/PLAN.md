...
- **S7 diff 审阅**：`filechanges.ts` + `diff.ts`。（验收：让 agent 在**同一条消息里**对同一文件发出两次 edit，两张卡片各自只显示该次 patch；同一条消息里两次 write 同一文件，两张卡片前后内容各自正确；重启 VS Code 恢复会话后，edit 卡片的 diff 仍可打开，write 卡片显示"本次会话不可用"。）**状态：已关闭（0.1.9，2026-09-15）** —— `edit` 按 `details.patch` 显示**补丁视图**（改动附近 4 行 + hunk 分隔）、`write` 用同名包装在互斥队列内抓前后内容（只活在进程内）；Mac（M1/M2）与 Windows（W0 `GATE PASS` / W1）都验过，发布核验逐字节一致（tag `v0.1.9`）。断言与发现见 `docs/S7-plan.md` §6/§11/§12。
...
- **S9 pi 包管理**：`Pi: Install Package` / `Pi: List Packages` / `Pi: Remove Package`。（Mac 上：安装 `../pi-config`（裸路径）后 `~/.pi/agent/settings.json` 出现该包，重启后其主题/扩展被加载。受限机上：用随扩展发布的 `test-fixtures/ext-smoke` 目录作为本地包源验证同一流程，因为该机无法拷入 pi-config；验收只要求"本次安装后重启可用"，扩展升级后失效属已知限制。输入 `npm:xxx` 得到明确的"需要 npm"错误。）
  ...
  > ①② 面板内对话框（聊天中的 select/confirm/input、面板发起的模型/思考/会话选择、API key 输入；
  > 命令面板入口与项目信任模态保留原生）。追加范围由 Astra 补审后才动代码。
