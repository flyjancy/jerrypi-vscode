...
**进行中**：**S9 实施期 · 步骤 2 完成**（`src/pi/packages.ts`：写路径信任边界 + 模块内串行化
（`withPackageLock`）+ 写后回读校验（`persistedSnapshot`/`drainErrors`）+ A18/A19/A20/A13b；host-check **357**）。
下一步：步骤 3（四个 VS Code 命令 + `package.json`，A5/A6/A8–A10/A16/A17）。
...
| `npm run self-test` | **9/9**（其中 host-check **357**、webview-dom 91、render 128、protocol 112、settings 16/16） |
...
| `node scripts/host-check.mjs` | **357** |
...
| 3 | 四个 VS Code 命令 + `package.json`（A5/A6/A8–A10/A16/A17） | ⏳ 下一个 |
