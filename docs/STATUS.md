...
**⏸ 暂停中（2026-09-17 用户决定暂停开发）**：**S9 编码十步全部落盘、自动门禁全绿**，卡在**人工验收**这一步；
工作区已提交（`git status` 干净）。恢复时从这里接着走（**顺序不能换**）：

1. **Mac M1/M2**（清单 `docs/S9-plan.md` §7；夹具已备好 `~/Desktop/s9-pkg-demo/`，含 `demo_echo` 与 `/demo-ask`）。
   按惯例 **F5 就行**（`preLaunchTask` 会 `sync + build`）；只有"面板默认在右侧"那一眼要开个**空窗口**看
   （F5 窗口与本仓库共用位置记忆）。面板找不到就用命令面板 `Pi: Focus Chat`。
2. **Marketplace 手动上传**（勾预发布）——**不要在 Mac 验收前上传**（本地就挂的包别发出去）。
3. `compare-vsix` 逐字节核验 + `git tag -a v0.1.13` + 推送。
4. **Windows W0/W1/W2**（`docs/S9-plan.md` §8）。
5. 回填 `docs/S9-plan.md` §12.2/§12.3，然后关阶段（STATUS 改回"已关闭"）。

**暂停时的快照**：版本 **0.1.13**（未上传）；`jerrypi-0.1.13.vsix` **338 文件 / 6,068,334 字节**、
SHA-256 `57fc7ad1…`（`check-vsix` OK；`unzip -l` 确认没有 `src/`、`scripts/`、`docs/`、`.pi/`、`AGENTS.md`）。
自动门禁见下面的表（`self-test 9/9` / `check:gate GATE PASS 18 项` / `check:controller 110/110`）。
**未验证的部分**（恢复后仍要真机看）：Windows 的 bash 探测链（W2）、老 profile 的面板位置记忆、
真焦点/真键盘/真输入法（M1⑥）。
...
| | |
| --- | --- |
| 阶段 | **S5、S6、S7、S8 均已关闭**；**S9 编码完成、暂停中**（待人工验收与发布；0.1.13 未上传；见上） |
...
| 工作区 | `main` 已推到 origin（**含 S9 的全部提交与暂停点**；S7 的提交与 `v0.1.9` tag 此前也已推）；工作区干净。**`v0.1.13` 还没打 tag / 没上传** —— 按计划等 Mac 验收通过后再打（tag 只标确实发布出去的产物）。**总计划只有一份**：`docs/PLAN.md`（2026-09-13 已把根目录那份的 233 行评审记录并进去并删除，见 S5-plan §10.1 的 U6） |
...
| 事 | 现状 | 记在哪 |
| --- | --- | --- |
| **S9 的人工验收与发布**（暂停点） | 编码完成、0.1.13 已打包并核过；**未做** Mac M1/M2、上传、tag、Windows W0/W1/W2 | S9-plan §7/§8/§12 · 本文 §1 的恢复清单 |
