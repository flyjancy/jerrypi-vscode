...
> **2026-09-17 实施期补记（用户真机踩到）**：把容器挪到 `secondarySidebar` 之后，**右侧辅助边栏如果从没打开过，
> 图标本身也不可见** —— 用户第一句话就是“找不到如何打开插件了”。README（中英）补了两行：
> ① 最快办法是命令面板 `Pi: Focus Chat`（它会展开面板所在边栏）；② 或 `View: Toggle Secondary Side Bar`
> 把右边栏打开。这是**文档缺口**（不是实现缺陷）：③ 的默认值本身正确，缺的是“怎么把它找出来”。
