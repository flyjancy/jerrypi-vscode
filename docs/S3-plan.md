...
| M5 | 大输出截断（`seq 1 4000`） | ✅ PASS（修了 1 处） | 首次 FAIL 是**重复**：pi 把 `[Showing lines 2001-4000 … Full output: …]` 写进正文，我们又渲染了"已截断 / 完整输出"两行 → 同一件事说三遍，而且那段脚注占掉折叠预览 5 个名额中的 2 个。pi 自己的 `renderResult` 就会把它剥掉（我们的两行正是它的 warning），已补上剥离 + 一条反向断言。修后：预览是干净的 5 行数字（3996–4000）、`还有 1995 行`、`已截断：显示 2000 / 共 4000 行`、完整输出路径可点 |
