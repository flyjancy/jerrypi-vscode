...
> 状态：**已关闭（2026-09-15，0.1.12）** —— 三轮评审全部吸收（§10.1/§10.2/§10.3），
> Mac（M1/M2）与 Windows（W0 `GATE PASS` / W1）都验过，发布核验逐字节一致（tag `v0.1.12`）。
> 实施期发现见 §11（含真机验收抓到的 3 条），结果见 §12。
...
### 11.2 回顾评审（**阶段关闭、0.1.12 发布之后**，2026-09-15）

S9 计划期间，另一个评审者（codex，`w60:pD` 面板）对**已关闭的 S8** 做了一次回顾审，
报了 4 条并给了复现。**我逐条自己复现过**（先写断言看红、再修，红/绿都记在最后一列）。

| # | 发现（我复现到的形态） | 根因 | 处置 | 能红验证 |
| --- | --- | --- | --- | --- |
| **R1** | 🔴 **"没有资源"被缓存成长期授权**：同一个窗口里先在空目录裁决（不问、按信任），之后目录里长出 `.pi/settings.json`，**下一个会话直接信任** ⇒ 项目级 `defaultTools` 等**未经询问就生效**（实测 `asked=0`、`isProjectTrusted()=true`） | `createTrustResolver` 在 `!hasRequiringResources` 分支里 `memo.set(cwd, true)` —— 把"当时没有资源"这件**事实**当成了"用户**同意**" | 该分支**不进 memo**（重新判定只是几次 `existsSync`）。断言 **A11⑦**（纯函数：之后必须问、且返回不信任）+ **A10⑪**（真 pi ×3：空目录不问 / 长出 `.pi/` 后必须问 / 不信任时项目配置不生效） | 把 `memo.set(cwd,true)` 塞回去 → A11⑦ + A10⑪ 两条红 |
| **R2** | **拒绝被中止改写**：用户先点「拒绝」，这一轮里再点「中止」⇒ `decision` 从 `deny` 变成 `cancelled`，卡片上"已拒绝"那个标记（**回放要用**）没了（实测 `fieldsOf` 从 `{"approval":"denied"}` 变 `{}`） | `settle()` 无条件覆盖已决记录，且答完之后 `ctx.signal` 上的 abort 监听**仍然挂着**（`{once:true}` 只保证"响一次"，不保证"不该响时不响"） | `settle` 幂等（**第一次结算说了算**）+ 结算时 `removeEventListener`。断言 **A6⑥** | 两处一起退回（= 原始实现）→ A6⑥ 红（单退一处会被另一处兜住，这点也记下来） |
| **R3** | **"清除记录、下次重新问"在父目录有记录时是假话**：父目录为 `true`、在子目录点清除 ⇒ 提示"下次会重新问"，**实际仍然信任、仍然不问**（实测 `get(child)=true`、`asked=0`） | 判定用会**向上找祖先**的 `get(cwd)`，而删除只删 `cwd` 自己的键 | 不动删除范围（祖先记录是用户数据、是在别的目录上做的决定），但**反馈说真话**：清完再读一次 `get(cwd)`，若非空则说明是上层目录（用 `getEntry()` 报出那个路径）+ 怎么彻底恢复。断言 **A11⑧** | 反馈退回旧文案 → A11⑧ 红 |
| **R4** | **上限没覆盖"取消"这条路径**：连续发起并中止 12 次（`maxDecided: 5`）⇒ `decided=12`，无界增长 | `pruneDecided()` 只挂在 `ask` 的早退与 `decide` 上；abort 监听这条路径只 `settle` 不淘汰 | 淘汰移进**统一结算路径**（`settle`），取消与拒绝/允许一视同仁。断言 **A6⑦** | 把 `pruneDecided()` 从 `settle` 里去掉 → A6⑦ 红 |

> 四条都**只影响"窗口内的一次性决定"这一层**，不涉及会话格式、不涉及用户数据（R1 的后果是"未经询问加载项目配置"，
> 属于安全问题但需要"用户先在一个空目录开过面板、之后目录里才出现资源"这个前提）。
> 修完 host-check **322 → 329**；**这四条随 S9 的 0.1.13 一起发**（不单独为此发一版：不需要任何新的人工动作，
> 断言全自动，Mac/Windows 的既有验收路径不受影响）。
...
**发布后的回顾评审（2026-09-15，codex，S8 已关闭）—— 4 条发现全部为真、全部已修**

| 项 | 结果 |
| --- | --- |
| R1 无资源被缓存成授权 | ✅ 已修（`memo` 不再缓存"没有资源"）＋ **A11⑦ / A10⑪**（真 pi 端到端） |
| R2 拒绝被中止改写 | ✅ 已修（`settle` 幂等 + 摘监听）＋ **A6⑥** |
| R3 "下次会重新问"是假话 | ✅ 已修（反馈说真话，报出上层目录）＋ **A11⑧** |
| R4 上限没覆盖取消路径 | ✅ 已修（淘汰移进统一结算）＋ **A6⑦** |
| 门禁影响 | host-check **322 → 329**；这四条随 **S9 的 0.1.13** 一起发（不需要新的人工动作） |
...
| **W1** | ✅ **PASS**：重启 VS Code 后打开面板**自动接过上一会话**（转写里还有历史），点一下会话框就能直接打字 |
...
**已拍板（2026-09-15）**：用户说"开工" ⇒ §4 的 **Q1–Q10 全按默认值执行**（那张表就是裁决记录）。
实施期没有临时改口；两处与 PLAN 原文不同、都在计划里显式裁决过：Q10（webview 销毁**不**取消待审批，
PLAN §5.3 那一句已按此改口）与 Q6（不做 `project_trust` 扩展事件）。
